// ado-automark.mjs — mark an Azure DevOps Test Case Passed from a finished run.
// A bare `az login` is not enough behind a corporate tenant; this uses the proven
// pattern: Entra bearer via `az account get-access-token --resource <ADO_RESOURCE_ID>`,
// auto `az login --tenant <your tenant>` on demand, ~50-min token cache, then the ADO
// REST lifecycle to mark a Test Case Passed + attach the .docx evidence + comment the
// work item. No PAT, ever (ADR-0007).
//
...
//
// Usage:
//   node ado-automark.mjs --test-case <id> --evidence "C:\\path\\to\\Evidence" [--plan <id>] [--suite 0] [--comment "Kontonummer: 12345"] [--attach "C:\\path\\clip.webm" --attach "C:\\path\\log.html"] [--dry-run]
...
//
// WHICH organisation, project, test plan and Entra tenant this talks to is NOT in this
// file and never will be: it is site configuration, and this source is public. It comes
// from ado-config.json beside the token cache, or from the environment — see CFG/cfgValue
// below and tools/README-ado-config.md. Nothing here has a default; an unconfigured
// install fails with the name of the setting it is missing.
...
// Microsoft's own, published application id for Azure DevOps — the resource a token is
// minted FOR. The same GUID for every Azure DevOps customer on earth; it identifies
// Microsoft's service, not anybody's tenant, and is therefore not site configuration.
...
/**
 * Where this install says which Azure DevOps it belongs to. Byte-identical to
 * de.ing.qa.studio.AdoConfig.configPath() in ingenious-plugin — one file, both languages.
 * Written once by the install; never committed. See tools/README-ado-config.md.
 */
export const CONFIG_FILE = join(CACHE_DIR, 'ado-config.json');

let _cfgFile;

function configFile() {
  if (_cfgFile) return _cfgFile;
  try { _cfgFile = JSON.parse(readFileSync(CONFIG_FILE, 'utf8')); } catch { _cfgFile = {}; }
  return _cfgFile;
}

/**
 * One setting, from the environment or from ado-config.json, and NO fallback.
 *
 * A default here would be somebody's organisation name in a public repository; and a
 * default that is wrong is worse than none, because it fails against a real Azure DevOps
 * with a 404 that reads like a broken test case rather than a missing setting.
 */
function cfgValue(envName, key) {
  const v = process.env[envName] ?? configFile()[key];
  if (v === undefined || v === null || String(v).trim() === '') {
    throw new Error(
      'Nicht eingerichtet: ' + key + ' fehlt. Bitte ' + CONFIG_FILE + ' anlegen '
      + '(Schluessel "' + key + '") oder die Umgebungsvariable ' + envName + ' setzen. '
      + 'Siehe tools/README-ado-config.md.');
  }
  return String(v).trim();
}

/** The site configuration, read on first use so that importing this module never throws. */
export const CFG = {
  get org() { return cfgValue('ADO_ORG', 'org'); },
  get project() { return cfgValue('ADO_PROJECT', 'project'); },
  get tenantId() { return cfgValue('ADO_TENANT_ID', 'tenantId'); },
  get planId() { return parseInt(cfgValue('ADO_TEST_PLAN_ID', 'planId'), 10); },
  // 0 = auto-discover, so this one has a meaningful default and needs no configuration.
  get suiteId() { return parseInt(process.env.ADO_TEST_SUITE_ID ?? configFile().suiteId ?? '0', 10); },
};
...
function cacheToken(token, expiresInSec) {
  try {
    ...
    writeFileSync(TOKEN_CACHE, JSON.stringify({ access_token: token, expires_at: Math.floor(Date.now() / 1000) + expiresInSec - 60 }), 'utf8');
...
function bearerHint() {
  return 'Set ADO_BEARER to a short-lived Entra token for the ADO resource '
    + ADO_RESOURCE_ID + ' (in Azure Pipelines: $(System.AccessToken)). No PAT — ADR-0007.';
}
...
export function getOrFetchToken() {
  ...
  if (needsLogin) {
    ...
    log('no active az session — opening `az login --tenant ' + CFG.tenantId + '` (browser)…');
    const login = runAz(['login', '--tenant', CFG.tenantId], 5 * 60 * 1000, true);
...
function cannedFor(method, url) {
  ...
  if (/\/testplan\/suites\?testCaseId=/.test(url)) return { value: [{ id: 3951634, plan: { id: CFG.planId } }] };
...
function selftestAuth() {
  ...
  const child = spawnSync(process.execPath, [fileURLToPath(import.meta.url), '--test-case', '3951650'],
  ...
  return {
    ...
    authHeadlessRulesOutPat: /No PAT/.test(childOut),
...
async function main() {
  ...
  if (a.selftest) {
    // The selftest supplies its own site configuration, and deliberately invented values:
    // it must pass on a machine that has never been configured, and it must never be able
    // to name a real organisation. Only set what the caller has not already set, so an
    // engineer can still point the selftest at their own values.
    process.env.ADO_ORG ??= 'selftest-org';
    process.env.ADO_PROJECT ??= 'selftest-project';
    process.env.ADO_TENANT_ID ??= '00000000-0000-0000-0000-000000000000';
    process.env.ADO_TEST_PLAN_ID ??= '1';
    dryPlan.length = 0;
    ...
    const res = await markPassed({
      testCaseId: 3951650, evidence: null, label: 'selftest', comment: 'Kontonummer: 000000 (synthetic)',
    ...
    const need = [
      /GET .*\/testplan\/suites\?testCaseId=3951650/,
      /GET .*\/Suites\/\d+\/TestPoint\?testCaseId=3951650/,
      /POST .*\/test\/runs\?/,
      /GET .*\/test\/runs\/99999\/results\?/,
      /GET .*\/wit\/workitems\/3951650\?/,
      /POST .*\/test\/runs\/99999\/results\?/, // PATCH-as-POST? no—we use PATCH; check below
    ];
    const has = (re) => urls.some((u) => re.test(u));
    const checks = {
      fastPathSuites: has(/GET .*\/testplan\/suites\?testCaseId=3951650/),
      testPoint: has(/GET .*\/Suites\/\d+\/TestPoint\?testCaseId=3951650/),
      createRun: has(/POST .*\/test\/runs\?/),
      getResults: has(/GET .*\/test\/runs\/99999\/results\?/),
      readSteps: has(/GET .*\/wit\/workitems\/3951650\?/),
      ...
      wiComment: has(/POST .*\/wit\/workItems\/3951650\/comments\?/),
      ...
      attachWorkItemRelations: dryPlan.filter((c) => c.method === 'PATCH' && /\/wit\/workitems\/3951650\?/.test(c.url) && Array.isArray(c.body) && c.body[0] && c.body[0].value && c.body[0].value.rel === 'AttachedFile').length === 2,
    ...
    // A fresh clone has no generated/ — create it rather than fail the selftest on it.
